...
class TestEnvStatic {
  ...
  static copyAsync(src, dest, options){
    ...
    return fse.copyAsync(src, dest, options)
      .catch(err => {
        debug('error', err)
        throw new TestEnvFsError('FS copy Failed', err)
      })
  }
  ...
  static existsAsync(test_path){
    ...
    return fse.existsAsync(test_path)
      .catch(err => {
        debug('error', err)
        throw new TestEnvFsError('FS exists Failed', err)
      })
  }
  ...
  static mkdirsAsync(src){
    ...
    return fse.mkdirsAsync(src)
      .catch(err => {
        debug('error', err)
        throw new TestEnvFsError('FS mkdir Failed', err)
      })
  }
  ...
  static removeAsync(rm_path){
    ...
    return fse.removeAsync(rm_path)
      .catch(err => {
        debug('error', err)
        throw new TestEnvFsError('FS remove Failed', err)
      })
  }
  ...
  static statAsync(test_path){
    ...
    return fse.statAsync(test_path)
      .catch(err => {
        debug('error', err)
        throw new TestEnvFsError('FS stat Failed', err)
      })
  }
