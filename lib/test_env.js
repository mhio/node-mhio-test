...
class TestEnv {
  ...
  static init( base ){
    ...
    // Where the tests at
    this.test_dir = 'test'
    this.test_path = path.join(this.base_path, this.test_dir)
  ...
  static setupTestDir( base_path, options = {} ){
    ...
    return new this(base_path, options)
  }
  ...
  constructor( base_path, options = {} ) {
    ...
    // Optionally fix a /test dir on the end
    if ( options.strip_test_dir ) {
      let dirs = base_path.split(path.sep)
      if ( dirs[0] === '' ) dirs[0] = '/'
      if ( dirs.pop() === this.test_dir ){
        base_path = path.join(...dirs)
        debug('Duplicate test_dir removed "%s"', base_path)
      }
    }
    ...
    debug('TestEnv base_path "%s"', base_path)
  ...
  cleanAsync(dir, options = {}){
    return new Promise((resolve, reject)=>{
      ...
      debug('clean removing dir "%s"', dir)
      return resolve(TestEnvStatic.removeAsync(dir).then(res => {
        debug('clean removed dir "%s"', dir)
        return res
      }))
    })
  }
  ...
  mkdirOutputAsync(...args){
    return new Promise((resolve) => {
      ...
      resolve(TestEnvStatic.mkdirsAsync(out_dir))
    })
  }

  // #### `mkdirOutputAsync(...args)`
  output(...args){
    return new TestEnvPathOutput(this, this.outputPath(...args))
  }

  // #### `mkdirOutputAsync(...args)`
  makeOutput(...args){
    return this.mkdirOutputAsync(...args).then(dir => {
      return new TestEnvPathOutput(this, dir)
    })
  }
  ...
  mkdirOutputTmpAsync(suffix, ...paths){
    return new Promise((resolve) => {
      ...
      return resolve(TestEnvStatic.mkdirsAsync(out_dir))
    })
  }

  // #### `mkdirOutputAsync(...args)`
  outputTmp(...args){
    return new TestEnvPathOutput(this, this.outputTmpPath(...args))
  }

  // #### `mkdirOutputAsync(...args)`
  makeOutputTmp(...args){
    return this.mkdirOutputTmpAsync(...args).then(dir => {
      return new TestEnvPathOutput(this, dir)
    })
  }
