...
const debug = require('debug')('mhio:test-helpers:TestEnv')
...
const { TestEnvException,
      TestEnvFsException } = require('./errors')
...
class TestEnv {
  ...
  static init( base ){
    ...
    // The guess removes `node_modules/@mhio/test/lib`.
    ...
    // alias
    this.setupTestDir = this.setupFromTestDirname
  ...
  // This should be removed - 0.5.0
  static trimDirNameFromPath(full_path, dir_name){
    console.warn('trimDirNameFromPath deprecated')
    let last = full_path.split(path.sep).pop()
    if ( last === dir_name ){
      full_path = path.normalize(path.join(full_path, '..'))
      debug('Removed "%s" from base path "%s"', dir_name, full_path)
    }
    return full_path
  }

  // Remove everything up to a directory from the end of a path, if it's there
  static trimUpToDirNameFromPath(full_path, dir_name_to_trim){
    let all = path.parse(full_path)
    let dirs = all.dir.split(path.sep)
    let delete_from = null
    for (let i = dirs.length - 1; i > -1; --i) {
      let current_dir = dirs[i]
      if ( current_dir === dir_name_to_trim ) delete_from = i
    }
    if ( delete_from !== null ) dirs = dirs.splice(0, delete_from)
    let new_path = path.join(all.root, ...dirs)
    debug('Removed up to "%s" from base path "%s" "%s"', dir_name_to_trim, full_path, new_path)
    return new_path
  }

  // Remove everything up to 2 directories from the end of a path, if it's there
  static trimUpToTwoDirNamesFromPath(full_path, dir_name_to_trim){
    let dirs = full_path.split(path.sep)
    let last = dirs.pop()
    let second_last = dirs.pop()
    if ( last === dir_name_to_trim ){
      full_path = path.normalize(path.join(full_path, '..'))
      debug('Removed "%s" from base path "%s"', dir_name_to_trim, full_path)
    }
    else if ( second_last === dir_name_to_trim ){
      full_path = path.normalize(path.join(full_path, '..', '..'))
      debug('Removed "%s" from base path "%s"', dir_name_to_trim, full_path)
    }
    return full_path
  ...
  static setupFromTestDirname( base_path, options = {} ){
  ...
  constructor( options = {} ) {
    ...
    // Go up to the app base path. "node_modules/@mhio/test/lib"
    // If we are not a direct dependency of the app we are testing then :/
    if (!options.base_path) {
      this.base_path = path.resolve(__dirname, '..', '..', '..', '..')
    } else {
      let base_path = ( options.base_path.join )
        ? path.join(...options.base_path)
        : options.base_path
    ...
    // Optionally remove the `test` or `test/x` dir from the end
    if ( options.strip_test_dir && this.base_path.split ) {
      this.base_path = TestEnv.trimUpToTwoDirNamesFromPath(this.base_path, this.test_dir_name)
    }
  ...
  cleanAsync(dir, options = {}){
    return new Promise((resolve, reject)=>{
      if (!dir)
        return reject(new TestEnvException('No dir to clean'))

      if (typeof dir !== 'string')
        return reject(new TestEnvException(`directory must be a string. ${typeof dir}`))

      // Be careful when deleting paths
      let base_path = this.basePath()
      if ( !dir.startsWith(base_path) && !options.force ) {
        return reject(new TestEnvException(`Can't clean outside of project without force option: ${dir}`))
      }

      // Be more careful when deleting paths
      if ( (dir === base_path || dir === `${base_path}/`) && !options.force ) {
        return reject(new TestEnvException(`Can't clean entire project without force option: ${dir}`))
      }
  ...
  removeAsync(dir, options = {}){
    return new Promise((resolve, reject)=>{
      if (!dir)
        return reject(new TestEnvException('No dir to remove'))
      ...
      let base_path = this.basePath()
      if ( !dir.startsWith(base_path) && !options.force ) {
        return reject(new TestEnvException(`Can't remove outside of project without force option: ${dir}`))
      }

      // Be more careful when deleting paths
      if ( (dir === base_path || dir === `${base_path}/`) && !options.force ) {
        return reject(new TestEnvException(`Can't remove entire project without force option: ${dir}`))
      }
  ...
  cleanOutputAsync(subdir){
    return new Promise((resolve) => {
      if (!subdir) throw new TestEnvException('No subdir to clean')
  ...
  copyFixtureToTmpOutputAsync(fixture_suffix){
    return new Promise.try(() => {
      ...
      return this.copyAsync(src, dest).then(()=> dest)
    })
  }
...
module.exports = { TestEnv, TestEnvPath, TestEnvException }
