...
class TestEnvPath {
  constructor( base_path, output = true ){
...
class TestEnv {
  ...
  static mkdirOutputAsync(...args){
    return new Promise((resolve) => {
      ...
      return resolve(fse.mkdirsAsync(out_dir).then(dir => new TestEnvPath(dir)))
    })
  }
  ...
  static copyFixtureToTmpOutputAsync(fixture_suffix){
    return new Promise((resolve) => {
      ...
      return this.copyAsync(src, dest).then(()=> resolve(new TestEnvPath(dest)))
    })
  }
